...
func newScheme(t *testing.T) *runtime.Scheme {
	...
	require.NoError(t, bomaragv1alpha1.AddToScheme(s))
	return s
}

// minimalCR returns a valid BomaRAG CR with the minimum required fields set.
func minimalCR(name, namespace string) *bomaragv1alpha1.BomaRAG {
	return &bomaragv1alpha1.BomaRAG{
		...
		Spec: bomaragv1alpha1.BomaRAGSpec{
			Frontend: bomaragv1alpha1.FrontendSpec{
				ComponentSpec: bomaragv1alpha1.ComponentSpec{Image: "bomalogic/bomarag-frontend:latest"},
			},
			Backend: bomaragv1alpha1.BackendSpec{
				ComponentSpec: bomaragv1alpha1.ComponentSpec{Image: "bomalogic/bomarag-backend:latest"},
			},
			Langflow: bomaragv1alpha1.LangflowSpec{
				ComponentSpec: bomaragv1alpha1.ComponentSpec{Image: "bomalogic/bomarag-langflow:latest"},
			},
		},
	}
}

func reconciler(s *runtime.Scheme, objs ...client.Object) (*BomaRAGReconciler, client.Client) {
	c := fake.NewClientBuilder().WithScheme(s).WithObjects(objs...).WithStatusSubresource(&bomaragv1alpha1.BomaRAG{}).Build()
	return NewBomaRAGReconciler(c, s), c
}

func reconcileOnce(t *testing.T, r *BomaRAGReconciler, cr *bomaragv1alpha1.BomaRAG) ctrl.Result {
	t.Helper()
...
func TestResourceName(t *testing.T) {
	assert.Equal(t, "bomarag-my-cr-fe", resourceName("my-cr", "fe"))
	assert.Equal(t, "bomarag-my-cr-be", resourceName("my-cr", "be"))
	assert.Equal(t, "bomarag-my-cr-lf", resourceName("my-cr", "lf"))
	assert.Equal(t, "bomarag-my-cr-docling-serve", resourceName("my-cr", "ds"))
	assert.Equal(t, "bomarag-my-cr-docling-worker", resourceName("my-cr", "dw"))
...
func TestSAName(t *testing.T) {
	assert.Equal(t, "bomarag-my-cr-fe", saName("my-cr", "fe"))
	assert.Equal(t, "bomarag-my-cr-be", saName("my-cr", "be"))
	assert.Equal(t, "bomarag-my-cr-lf", saName("my-cr", "lf"))
}
...
func TestReconcile_FrontendEnvContainsBackendHost(t *testing.T) {
	...
	for _, e := range d.Spec.Template.Spec.Containers[0].Env {
		if e.Name == "BOMARAG_BACKEND_HOST" {
			backendHost = e.Value
		}
	}
...
func TestReconcile_BackendEnvContainsLangflowURL(t *testing.T) {
	...
	assert.Contains(t, envContent, `BOMARAG_BACKEND_INTERNAL_URL=http://`+instanceResourceName(cr, "be")+`:8000`)
}
...
func TestReconcile_LangflowMountsPVC(t *testing.T) {
	...
	cr.Spec.Langflow.Storage = &bomaragv1alpha1.PersistenceSpec{Enabled: true, Size: size}
	r, c := reconciler(s, cr)
...
func TestReconcile_CreatesTargetNamespace(t *testing.T) {
	s := newScheme(t)
	cr := minimalCR("my-bomarag", "operator-ns")
	cr.Spec.TargetNamespace = "tenant-ns"
	r, c := reconciler(s, cr)
...
func TestReconcile_AddsFinalizer_WhenTargetNamespaceDiffers(t *testing.T) {
	...
	updated := &bomaragv1alpha1.BomaRAG{}
	require.NoError(t, c.Get(context.Background(),
...
func TestReconcile_SkipsNamespaceCreation_WhenSameAsCR(t *testing.T) {
	...
	for _, ns := range nsList.Items {
		assert.NotEqual(t, "bomarag-operator", ns.Labels["app.kubernetes.io/managed-by"],
...
func TestMergeDeploymentLabels_CustomLabelsAreMerged(t *testing.T) {
	...
	assert.Equal(t, "bomarag", merged["app.kubernetes.io/name"])
...
func TestReconcile_FrontendCustomPodLabels(t *testing.T) {
	...
	assert.Equal(t, "bomarag", podLabels["app.kubernetes.io/name"])
	assert.Equal(t, "my-bomarag", podLabels["app.kubernetes.io/instance"])
	assert.Equal(t, "fe", podLabels["app.kubernetes.io/component"])
	assert.Equal(t, "bomarag-operator", podLabels["app.kubernetes.io/managed-by"])
}
...
func TestReconcile_SelectorLabelsAreNotAffectedByCustomPodLabels(t *testing.T) {
	...
	assert.Equal(t, "bomarag", selectorLabels["app.kubernetes.io/name"])
	assert.Equal(t, "my-bomarag", selectorLabels["app.kubernetes.io/instance"])
	assert.Equal(t, "fe", selectorLabels["app.kubernetes.io/component"])
	assert.Equal(t, "bomarag-operator", selectorLabels["app.kubernetes.io/managed-by"])
...
func TestReconcile_FrontendDeploymentLevelLabelsAndAnnotations(t *testing.T) {
	...
	assert.Equal(t, "bomarag", d.Labels["app.kubernetes.io/name"])
...
// The bomarag- prefix ensures names always start with a letter.
func TestReconcile_InstanceName_UsedInResourceNames(t *testing.T) {
	...
	for _, role := range []string{"fe", "be", "lf"} {
		...
		assert.Equal(t, "bomarag-prod-"+role, name)
...
func TestReconcile_CustomServiceName_OperatorCreates(t *testing.T) {
	...
	assert.Contains(t, envContent, `BOMARAG_BACKEND_INTERNAL_URL=http://my-backend-svc:8000`,
...
func TestReconcile_CustomServiceName_UsedInFrontendEnv(t *testing.T) {
	...
	for _, env := range deploy.Spec.Template.Spec.Containers[0].Env {
		if env.Name == "BOMARAG_BACKEND_HOST" {
...
func TestEnvHash_StableAcrossReconciles(t *testing.T) {
	...
	s := newScheme(t)
	cr := minimalCR("test-bomarag", "test-ns")
...
func TestDeployment_ContainsEnvHashAnnotation(t *testing.T) {
	...
	assert.Contains(t, annotations, "bomalogic.com/backend-env-hash", "Backend pod should have env hash annotation")
	assert.NotEmpty(t, annotations["bomalogic.com/backend-env-hash"], "Hash annotation should not be empty")
	assert.Len(t, annotations["bomalogic.com/backend-env-hash"], 64, "Hash should be 64 hex characters")
	...
	assert.Contains(t, lfAnnotations, "bomalogic.com/langflow-env-hash", "Langflow pod should have env hash annotation")
	assert.NotEmpty(t, lfAnnotations["bomalogic.com/langflow-env-hash"], "Hash annotation should not be empty")
	assert.Len(t, lfAnnotations["bomalogic.com/langflow-env-hash"], 64, "Hash should be 64 hex characters")
}
...
func TestDeployment_HashChangeTriggersUpdate(t *testing.T) {
	...
	hash1 := backendDeploy1.Spec.Template.Annotations["bomalogic.com/backend-env-hash"]
	require.NotEmpty(t, hash1, "Initial hash should exist")

	// Update CR with different env value
	updatedCR := &bomaragv1alpha1.BomaRAG{}
	require.NoError(t, c.Get(context.Background(),
		types.NamespacedName{Name: "test-bomarag", Namespace: "test-ns"}, updatedCR))
	...
	hash2 := backendDeploy2.Spec.Template.Annotations["bomalogic.com/backend-env-hash"]
