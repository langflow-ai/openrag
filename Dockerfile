...
RUN echo '#!/bin/bash' > /usr/share/opensearch/setup-security.sh && \
    ...
    echo 'BACKEND_URL=${BOMARAG_BACKEND_INTERNAL_URL:-http://${BOMARAG_BACKEND_HOST:-bomarag-backend}:${BOMARAG_BACKEND_PORT:-8000}}' >> /usr/share/opensearch/setup-security.sh && \
    echo 'sed -i "s|http://bomarag-backend:8000|$BACKEND_URL|g" /usr/share/opensearch/securityconfig/config.yml /usr/share/opensearch/cloud_securityconfig/config.yml' >> /usr/share/opensearch/setup-security.sh && \
