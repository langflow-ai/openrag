...
The first time you start the BomaRAG application, you must complete the application onboarding process to select language and embedding models that are essential for BomaRAG features like the [**Chat**](/chat).
...
Additionally, you can select multiple embedding models.
...
1. Enter your Anthropic API key, or enable **Use environment API key** to pull the key from your [BomaRAG `.env` file](/reference/configuration).
...
5. Click **Complete**.
   ...
   After you configure the embedding model, BomaRAG uses your credentials and models to ingest some [initial documents](/knowledge-browse#default-documents). This tests the connection, and it allows you to ask BomaRAG about itself in the [**Chat**](/chat).
...
6. Continue through the overview slides for a brief introduction to BomaRAG, or click <Icon name="ArrowRight" aria-hidden="true"/> **Skip overview**.
The overview demonstrates some basic functionality that is covered in the [quickstart](/quickstart#chat-with-documents) and in other parts of the BomaRAG documentation.
...
2. Enter your watsonx.ai deployment's project ID and API key.
   ...
   You can enable **Use environment API key** to pull the key from your [BomaRAG `.env` file](/reference/configuration).
...
7. Continue through the overview slides for a brief introduction to BomaRAG, or click <Icon name="ArrowRight" aria-hidden="true"/> **Skip overview**.
The overview demonstrates some basic functionality that is covered in the [quickstart](/quickstart#chat-with-documents) and in other parts of the BomaRAG documentation.
...
However, it requires additional setup because Ollama isn't included with BomaRAG.
...
1. [Install Ollama locally or on a remote server](https://docs.ollama.com/), or [run models in Ollama Cloud](https://docs.ollama.com/cloud).
   ...
   If you are running a remote server, it must be accessible from your BomaRAG deployment.

2. In the BomaRAG onboarding dialog, enter your Ollama server's base URL:
...
3. Select a language model that your Ollama server is running.
   ...
   BomaRAG only lists language models that support tool calling.
...
1. Enter your OpenAI API key, or enable **Use environment API key** to pull the key from your [BomaRAG `.env` file](/reference/configuration).
