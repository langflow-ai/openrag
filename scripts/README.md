...
## Bulk ingest (`bomarag_bulk.py`)

`bomarag_bulk.py` is a standalone client for loading a directory or a set of
files through BomaRAG's public ingestion API. It groups files into server tasks,
...
The script has inline `uv` dependency metadata and does not import the BomaRAG
application or SDK, so it can also be copied and run outside this repository.

Set the URL of the BomaRAG frontend and an API key with `knowledge:upload` and
`knowledge:read:own` permissions:

```bash
export BOMARAG_URL=http://localhost:3000
export BOMARAG_API_KEY=orag_your_api_key

uv run scripts/bomarag_bulk.py bulk upload ./documents \
  --include '*.pdf' \
  --exclude 'archive/*' \
  --batch-size 10 \
  --max-inflight 4 \
...
`--max-inflight` bounds the number of submitted BomaRAG tasks, including tasks
...
Runs are stored under `~/.bomarag/bulk/runs` by default. They can be inspected
without a running server or an API key:

```bash
uv run scripts/bomarag_bulk.py bulk list
uv run scripts/bomarag_bulk.py bulk summary
uv run scripts/bomarag_bulk.py bulk summary --detail latest
```
...
The same standalone client includes the arXiv benchmark from `bomarag-lite`.
...
caches the tarball, extracts the selected PDFs, and uploads them to BomaRAG:

```bash
export BOMARAG_URL=http://localhost:3000
export BOMARAG_API_KEY=orag_your_api_key

uv run scripts/bomarag_bulk.py bench arxiv \
  --max-results 100 \
  --batch-size 10 \
  --max-inflight 4
```
...
Download and cache PDFs without connecting to BomaRAG by passing
...
The PDF cache defaults to `~/.bomarag/benchmarks/arxiv/pdfs`. Existing PDFs and
...
Benchmark run records default to `~/.bomarag/benchmarks/arxiv/runs`:

```bash
uv run scripts/bomarag_bulk.py bench arxiv list
uv run scripts/bomarag_bulk.py bench arxiv summary
uv run scripts/bomarag_bulk.py bench arxiv summary --detail latest
```

## Sync default user roles (`sync_default_user_roles.py`)

Dev-only helper for RBAC in **OSS run mode** (`BOMARAG_RUN_MODE=oss`): updates
existing users in the SQL DB when `BOMARAG_DEFAULT_ROLE` changes. Requires:

```env
BOMARAG_RUN_MODE=oss
BOMARAG_SYNC_DEFAULT_ROLE=true
BOMARAG_DEFAULT_ROLE=admin   # target role: admin | developer | user | viewer
```

Ignored in `saas` and `on_prem` — those modes assign roles from JWT claims.

Uses the default SQLite DB at `data/bomarag.db` unless `DATABASE_URL` is set.

### Promote all `user` roles to `BOMARAG_DEFAULT_ROLE`

Use this when users still have the `user` role but you want them on whatever
role is set in `BOMARAG_DEFAULT_ROLE` (for example `admin`):

```bash
BOMARAG_SYNC_DEFAULT_ROLE=true \
BOMARAG_DEFAULT_ROLE=admin \
...
what `BOMARAG_DEFAULT_ROLE` is set to.

What it does:

- Finds every user whose **only** role is `user`
- Assigns them the role from `BOMARAG_DEFAULT_ROLE` (here: `admin`)
...
Replace `user` with any source role, and set `BOMARAG_DEFAULT_ROLE` to the
...
| `--to-role ROLE` | Target role (overrides `BOMARAG_DEFAULT_ROLE`; requires `--from-role`) |
| `--from-noauth-role ROLE` | Source role for the anonymous user |
| `--to-noauth-role ROLE` | Target role for anonymous user (overrides `BOMARAG_NOAUTH_ROLE`) |
| `--record-baseline` | Save current env defaults; do not change any user |

### After running

Restart the backend (or wait for `BOMARAG_PERM_CACHE_TTL`, default 60s) so
...
(`BOMARAG_STORAGE_MODE=db` by default):
