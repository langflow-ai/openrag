...
Upload documents to your [BomaRAG OpenSearch knowledge base](/knowledge) to populate your knowledge base with unique content, such as your own company documents, research papers, or websites.

BomaRAG can ingest knowledge from direct file uploads, URLs, and cloud storage connectors.

Knowledge ingestion is powered by BomaRAG's built-in knowledge ingestion flows that use Docling to process documents before storing the documents in your OpenSearch knowledge base.
...
BomaRAG has verified (tested) support for the following file types:
...
You can submit an [BomaRAG GitHub issue](https://github.com/ABISHAIMWANJA/bomarag/issues) to request support for specific file types or report ingestion errors.
...
2. Click **Add Knowledge** to add your own documents to your BomaRAG knowledge base.
...
3. To upload one file, click <Icon name="File" aria-hidden="true"/> **File**. To upload all documents in a folder, click <Icon name="Folder" aria-hidden="true"/> **Folder**.
   ...
   The default path is `~/.bomarag/documents`.
...
To ingest knowledge from cloud storage using an BomaRAG cloud storage connector, do the following:
...
On the <Icon name="Library" aria-hidden="true"/> **Knowledge** or <Icon name="Settings2" aria-hidden="true"/> **Settings** page, click <Icon name="Settings2" aria-hidden="true"/> **Sync** to synchronize your cloud storage and your BomaRAG knowledge base.

## Ingest knowledge from URLs {#url-flow}

When using the BomaRAG chat, you can enter URLs into the chat to be ingested in real-time during your conversation.

BomaRAG runs the **OpenSearch URL Ingestion** flow to ingest web content from URLs.
This flow isn't directly accessible from the BomaRAG user interface, and it doesn't use Docling.
Instead, this flow is called by the [**BomaRAG OpenSearch Agent** flow](/chat#flow) as a [Model Context Protocol (MCP) tool in Langflow](https://docs.langflow.org/mcp-tutorial).
...
:::tip
Like all BomaRAG flows, you can [inspect the flow in Langflow](/agents#inspect-and-modify-flows), and you can customize it.

To ingest URLs recursively, edit the **Depth** parameter in the **OpenSearch URL Ingestion** flow.
:::

The BomaRAG chat cannot ingest URLs that end in static document file extensions like `.pdf`.
...
In the BomaRAG user interface, a badge is shown on <Icon name="Bell" aria-hidden="true"/> **Tasks** when BomaRAG tasks are active.
...
On a local VM with 7 vCPUs and 8 GiB RAM, BomaRAG ingested approximately 5.03 GB across 1,083 files in about 42 minutes.
