...
1. Mirror the following images to your registry.
   ...
   | `docker.io/opensearchproject/opensearch-dashboards:3.8.0` | `DASHBOARDS_REGISTRY/opensearchproject/opensearch-dashboards:3.8.0` |

   The first four images resolve from `IMAGE_REGISTRY` and `IMAGE_ORG`, so you can mirror them into any organization you control.
   The OpenSearch Dashboards image resolves from `DASHBOARDS_REGISTRY` only, so you must preserve the `opensearchproject/opensearch-dashboards` path and the `3.8.0` tag when you mirror it.
