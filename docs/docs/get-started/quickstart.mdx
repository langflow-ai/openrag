...
:::tip Python installation is automatic
When the script runs `uvx openrag`, `uv` automatically downloads and installs Python 3.10-3.13 if it's not already on your system. You don't need Python pre-installed when using the automatic installer, but having it pre-installed is recommended for better reliability.
:::
