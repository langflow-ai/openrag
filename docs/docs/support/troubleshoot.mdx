...
`easyocr` is already included as a dependency in OpenRAG's `pyproject.toml`. If you're running OpenRAG with `uvx openrag`, `uvx` creates a cached, ephemeral environment that may become stale.
