---
name: bomarag_sdk
description: Guide developers through integrating the BomaRAG SDK into applications with code examples, configuration, and best practices
---

When the user asks to integrate the BomaRAG SDK or use BomaRAG in their application, follow this workflow.

## Initial assessment phase
Before starting SDK integration:
1. Identify the BomaRAG instance:
...
### Python SDK
**Package:** [`bomarag-sdk`](https://pypi.org/project/bomarag-sdk/)
...
**Package:** [`bomarag-sdk`](https://libraries.io/npm/bomarag-sdk)
...
### MCP Server
**Package:** [`bomarag-mcp`](https://pypi.org/project/bomarag-mcp/)

For MCP integration (Model Context Protocol):
```bash
pip install bomarag-mcp
```
...
```bash
BOMARAG_API_KEY=your-api-key  # Required if authentication is enabled
BOMARAG_URL=http://localhost:3000  # Optional, defaults to localhost:3000
```
...
# Using environment variables (auto-discovers BOMARAG_API_KEY and BOMARAG_URL)
...
async function handleErrors() {
  try {
    ...
    } else if (error instanceof BomaRAGError) {
...
const client = new BomaRAGClient({
  baseUrl: process.env.REACT_APP_BOMARAG_URL
});
...
3. **Validate and sanitize inputs**
   - Validate user inputs before sending to BomaRAG
...
- Check network connectivity if BomaRAG is on a remote server
...
2. **Health checks**
   ...
   - Monitor BomaRAG service availability
...
4. **Fallback handling**
   - Implement graceful degradation if BomaRAG is unavailable
...
- [ ] Client successfully connects to BomaRAG
...
- **Python SDK:** https://pypi.org/project/bomarag-sdk/
- **TypeScript SDK:** https://libraries.io/npm/bomarag-sdk
- **MCP Server:** https://pypi.org/project/bomarag-mcp/
- **GitHub Repository:** https://github.com/ABISHAIMWANJA/bomarag/tree/main/sdks
- **Official Documentation:** https://docs.bomarag.com
...
- Be explicit about what requires BomaRAG to be running
