...
class OpenSearchVectorStoreComponentMultimodalMultiEmbedding(LCVectorStoreComponent):
    ...
    description: str = (
        ...
        '  {search_query: "components in bomarag"}'
    )
    ...
    _bomarag_ingest_global_placeholders = {
    ...
    inputs = [
        ...
        *LCVectorStoreComponent.inputs,  # includes search_query, add_documents, etc.
        ...
        DropdownInput(
            ...
            value="bomarag",
            options=["basic", "jwt", "bomarag"],
            info=(
                "Authentication method: 'basic' for username/password authentication, "
                "'jwt' for JSON Web Token (Bearer) authentication, or 'bomarag' to "
                "delegate writes to the BomaRAG backend ingest callback (no direct "
                "OpenSearch credentials required — only BOMARAG_* fields)."
            ),
        ...
        StrInput(
            name="bomarag_ingest_url",
            display_name="BomaRAG Ingest URL",
            value="BOMARAG_INGEST_URL",
            load_from_db=True,
            input_types=["Text", "Message"],
            advanced=True,
            info="Internal BomaRAG callback URL for backend-owned document indexing.",
        ),
        SecretStrInput(
            name="bomarag_ingest_token",
            display_name="BomaRAG Ingest Token",
            value="BOMARAG_INGEST_TOKEN",
            load_from_db=True,
            input_types=["Text", "Message"],
            advanced=True,
            info="Short-lived token used only for BomaRAG ingest callbacks.",
        ),
        StrInput(
            name="bomarag_ingest_run_id",
            display_name="BomaRAG Ingest Run ID",
            value="BOMARAG_INGEST_RUN_ID",
            load_from_db=True,
            input_types=["Text", "Message"],
            advanced=True,
        ),
        IntInput(
            name="bomarag_ingest_batch_size",
            display_name="BomaRAG Ingest Batch Size",
            value=100,
            advanced=True,
        ),
    ]
    ...
    @staticmethod
    def _bomarag_input_to_str(value: Any) -> str:
        if value is None:
            return ""
    ...
    def _bomarag_callback_value(self, attr_name: str) -> str:
        value = self._bomarag_input_to_str(getattr(self, attr_name, ""))
        if value == self._bomarag_ingest_global_placeholders.get(attr_name):
            return ""
        return value

    def _bomarag_ingest_callback_config(self) -> tuple[str, str, str] | None:
        url = self._bomarag_callback_value("bomarag_ingest_url")
        token = self._bomarag_callback_value("bomarag_ingest_token")
        ...
        if not url or not token or not ingest_run_id:
            msg = "BomaRAG ingest callback requires url, token, and ingest_run_id."
            raise ValueError(msg)

        logger.debug(
            "[BomaRAG callback config] configured for ingest_run_id=%s (url_len=%d, token_len=%d)",
            ingest_run_id,
            len(url),
            len(token),
        )
    ...
    def _post_bomarag_ingest_batches(
        self,
        *,
        requests: list[dict],
        vector_field: str,
        ...
        callback_config = self._bomarag_ingest_callback_config()
        if callback_config is None:
            return

        url, token, ingest_run_id = callback_config
        batch_size = max(self._parse_int_param("bomarag_ingest_batch_size", 100), 1)
        timeout = self._parse_int_param("request_timeout", REQUEST_TIMEOUT)
        headers = {"Authorization": f"Bearer {token}"}

        logger.debug(
            f"[BomaRAG ingest POST] total_chunks={len(requests)} batch_size={batch_size} timeout_s={timeout}"
        )
        ...
        with httpx.Client(timeout=timeout) as client:
            ...
            for batch_number, start in enumerate(range(0, len(requests), batch_size), start=1):
                ...
                logger.debug(
                    f"[BomaRAG ingest POST] -> batch={batch_number}/{total_batches} "
                    f"chunks={len(payload['chunks'])} final={final}"
                )
                response = client.post(url, json=payload, headers=headers)
                logger.debug(f"[BomaRAG ingest POST resp] batch={batch_number} status={response.status_code}")
                if response.status_code >= 400:
                    msg = (
                        "BomaRAG ingest callback failed "
                        f"(batch={batch_number}, status={response.status_code})"
                    )
                    logger.warning(msg)
                    raise RuntimeError(msg)

        logger.debug(f"Posted {len(requests)} chunks to BomaRAG backend ingest callback.")

    @staticmethod
    def _bomarag_chunk_payload(
        request: dict,
        *,
        vector_field: str,
        text_field: str,
    ...
    def _bulk_ingest_embeddings(
        ...
        *,
        ...
        if self._bomarag_ingest_callback_config() is not None:
            self._post_bomarag_ingest_batches(
                requests=requests,
                vector_field=vector_field,
                text_field=text_field,
            )
    ...
    def _build_auth_kwargs(self) -> dict[str, Any]:
        ...
        if mode == "bomarag":
            # Writes are delegated to the BomaRAG backend ingest callback,
            # so no direct OpenSearch credentials are needed. Only the
            # BOMARAG_* fields are required for ingestion to function.
            url = self._bomarag_callback_value("bomarag_ingest_url")
            token = self._bomarag_callback_value("bomarag_ingest_token")
            ingest_run_id = self._bomarag_callback_value("bomarag_ingest_run_id")
            missing = [
                name
                for name, value in (
                    ("bomarag_ingest_url", url),
                    ("bomarag_ingest_token", token),
                    ("bomarag_ingest_run_id", ingest_run_id),
                )
                if not (value or "").strip()
            ]
            if missing:
                msg = (
                    "Auth Mode is 'bomarag' but required BOMARAG_* fields are "
                    f"missing: {', '.join(missing)}."
                )
                raise ValueError(msg)
            return {}
        user = (self.username or "").strip()
        pwd = (self._bomarag_input_to_str(self.password) or "").strip()
    ...
    def _add_documents_to_vector_store(self, client: OpenSearch) -> None:
        ...
        use_bomarag_ingest_callback = self._bomarag_ingest_callback_config() is not None
        ...
        if use_bomarag_ingest_callback:
            self.log("Using BomaRAG backend ingest callback; skipping direct OpenSearch writes.")
        else:
            # Check for AOSS
            auth_kwargs = self._build_auth_kwargs()
    ...
    async def update_build_config(
        ...
        try:
            if field_name == "auth_mode":
                ...
                # In 'bomarag' mode, expose the BOMARAG_* fields up front
                # since they are the only credentials required.
                for bomarag_field in (
                    "bomarag_ingest_url",
                    "bomarag_ingest_token",
                    "bomarag_ingest_run_id",
                    "bomarag_ingest_batch_size",
                ):
                    if bomarag_field in build_config:
                        build_config[bomarag_field]["advanced"] = not is_bomarag
                        build_config[bomarag_field]["required"] = (
                            is_bomarag and bomarag_field != "bomarag_ingest_batch_size"
                        )

                if is_basic or is_bomarag:
