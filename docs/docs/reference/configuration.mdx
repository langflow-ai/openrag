...
| `LANGFUSE_BASE_URL` | Not set | Preferred endpoint variable for Langfuse. Leave empty for Langfuse Cloud. Required for self-hosted Langfuse deployments if `LANGFUSE_SECRET_KEY` and `LANGFUSE_PUBLIC_KEY` are set. The address must be relative to the OpenRAG container deployment. For example, `http://localhost:3002` or `http://host.docker.internal:3000`. If both endpoint variables are set, `LANGFUSE_BASE_URL` takes precedence. |
| `LANGFUSE_HOST` | Not set | Deprecated endpoint alias retained for backward compatibility. It is used when `LANGFUSE_BASE_URL` is not set. |
