...
from scripts import bomarag_bulk as bulk
...
@pytest.mark.asyncio
async def test_api_client_posts_repeated_file_parts_to_bomarag_v1(tmp_path: Path) -> None:
    ...
    client = bulk.BomaRAGApiClient(
        base_url="http://bomarag.test",
        api_key="secret-key",
    ...
    assert captured["url"] == "http://bomarag.test/api/v1/documents/ingest"
...
def test_bulk_upload_command_loads_a_directory(
    ...
    monkeypatch.setattr(bulk, "BomaRAGApiClient", FakeClient)
    exit_code = bulk.main(
        [
            "bulk",
            "upload",
