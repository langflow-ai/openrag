...
a dimension containing `"*"` collapses to `[]` (i.e. "no filter on this field").
"""
...
def _strip_wildcards(filters: dict[str, Any] | None) -> dict[str, list[str]]:
    """Drop wildcard-containing and empty lists from each filter dimension."""
    if not filters:
        return {}
...
def merge_filter_overrides(
    ...
    their value. This lets callers intentionally set defaults like `limit=10`,
