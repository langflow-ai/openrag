...
| `WATSONX_ONPREM_ENDPOINT` | Not set | HTTPS base URL of the IBM Cloud Pak for Data or Software Hub cluster. |
| `WATSONX_ONPREM_USERNAME` | Not set | Cluster username used with `WATSONX_ONPREM_API_KEY`. |
| `WATSONX_ONPREM_API_KEY` | Not set | Cluster API key exchanged with the username for CPD authorization. |
| `WATSONX_ONPREM_ZEN_API_KEY` | Not set | Pre-encoded `base64(username:api-key)` alternative to the username and API-key variables. |
| `WATSONX_ONPREM_SPACE_ID` | Not set | Deployment space used for inference. |
| `WATSONX_ONPREM_PROJECT_ID` | Not set | Optional project alternative to a deployment space. |
| `WATSONX_ONPREM_TLS_VERIFY` | `false` | Provider-scoped TLS policy: `false`, `true`, or a mounted CA bundle path. `SSL_VERIFY` no longer controls this provider; `SSL_CERT_FILE` supplies the process trust store when verification is enabled. |
