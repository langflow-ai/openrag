...
async def ensure_new_field_exists(
    client: AsyncOpenSearch, index_name: str, field_name: str, dimensions: int
) -> None:
...
async def find_legacy_documents(
    client: AsyncOpenSearch, index_name: str, batch_size: int = 100
) -> list[dict[str, Any]]:
    """Find all documents with legacy chunk_embedding but no embedding_model field."""
    query = {
        "query": {
            "bool": {
                "must": [{"exists": {"field": "chunk_embedding"}}],
...
async def migrate_documents(
    ...
    for doc in documents:
        ...
        update_doc = {new_field_name: legacy_embedding, "embedding_model": model_name}

        action = {"_op_type": "update", "_index": index_name, "_id": doc_id, "doc": update_doc}
...
async def migrate_legacy_embeddings(
    model_name: str, batch_size: int = 100, dry_run: bool = False, index_name: str = None
) -> bool:
    ...
    try:
        ...
        result = await migrate_documents(
            client, index_name, legacy_docs, model_name, new_field_name, dry_run
        )
...
Examples:
  ...
  uv run python scripts/migrate_embedding_model_field.py --model nomic-embed-text --batch-size 500
    ...
    parser.add_argument("--index", default=None, help=f"Index name (default: {get_index_name()})")
