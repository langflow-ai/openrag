"""OpenAI-compatible chat model that talks to the BomaRAG `/v1` proxy.

Langflow never holds upstream vendor keys. At runtime BomaRAG injects:

- ``BOMARAG_LLM_TOKEN`` — short-lived hop token (Authorization Bearer)
- ``BOMARAG_LLM_BASE_URL`` — ``http://<backend>/v1``
...
class OpenAICompatibleLLMComponent(LCModelComponent):
    ...
    inputs = [
        ...
        SecretStrInput(
            ...
            info="Hop token from BOMARAG_LLM_TOKEN. BomaRAG injects this per Langflow run.",
