...
class TestAuth:
    ...
    def test_missing_api_key_and_extra_headers_raises_at_construction(self):
        ...
        nor extra_headers are supplied (and no BOMARAG_API_KEY env var is set)."""
        from bomarag_sdk import BomaRAGClient
        from bomarag_sdk.exceptions import AuthenticationError

        env_backup = os.environ.pop("BOMARAG_API_KEY", None)
        try:
            ...
            if env_backup is not None:
                os.environ["BOMARAG_API_KEY"] = env_backup
    ...
    async def test_extra_headers_alone_satisfies_auth_check(self):
        ...
        extra_headers instead of the BOMARAG_API_KEY env var or api_key argument.
    ...
    def test_env_var_api_key_satisfies_auth_check(self):
        """Client must not raise when only the BOMARAG_API_KEY env var is set."""
        from bomarag_sdk import BomaRAGClient

        env_backup = os.environ.pop("BOMARAG_API_KEY", None)
        try:
            os.environ["BOMARAG_API_KEY"] = "orag_env_var_key"
            client = BomaRAGClient(base_url=_base_url)
            assert client is not None
        finally:
            del os.environ["BOMARAG_API_KEY"]
            if env_backup is not None:
                os.environ["BOMARAG_API_KEY"] = env_backup

    def test_explicit_api_key_takes_precedence_over_env_var(self):
        """Explicit api_key argument overrides the BOMARAG_API_KEY env var."""
        from bomarag_sdk import BomaRAGClient

        env_backup = os.environ.pop("BOMARAG_API_KEY", None)
        try:
            os.environ["BOMARAG_API_KEY"] = "orag_env_var_key"
            client = BomaRAGClient(api_key="orag_explicit_key", base_url=_base_url)
    ...
    async def test_invalid_api_key_raises_auth_error(self):
        ...
        bad_client = BomaRAGClient(api_key="orag_invalid_key_for_testing", base_url=_base_url)
        try:
            with pytest.raises(AuthenticationError) as exc_info:
