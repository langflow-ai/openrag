...
Because the repository path and tag for that image are fixed, confirm that the mirrored image is available at `DASHBOARDS_REGISTRY/opensearchproject/opensearch-dashboards:3.8.0`, and make sure `DASHBOARDS_REGISTRY` is set correctly.
