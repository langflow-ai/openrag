...
test.describe(`Financial Document - ${ACTIVE_PROVIDER_CONFIG.provider} @33219232`, () => {
  test("Table Structure ON - document Q&A", async ({
    ...
    logger.info(
      `\n🧪 Testing Financial Document with ${ACTIVE_PROVIDER_CONFIG.provider}`,
    );
    ...
    logger.info(
      `  ✓ Language model set to: ${ACTIVE_PROVIDER_CONFIG.language}`,
    );
    logger.info(
