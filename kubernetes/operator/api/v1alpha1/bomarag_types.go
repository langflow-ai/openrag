...
// PVCReclaimPolicy describes what happens to PVCs when the BomaRAG CR is deleted.
...
type ComponentSpec struct {
	...
	// These are merged with the global imagePullSecrets from the BomaRAG spec.
	// +optional
	...
	// If not specified, defaults to "bomarag-{role}" (e.g., "bomarag-fe", "bomarag-be", "bomarag-lf").
	// +optional
	...
	// Merged with operator-managed labels and CommonResourceLabels from BomaRAGSpec.
	...
	// Merged with CommonResourceAnnotations from BomaRAGSpec.
...
// FrontendSpec configures the BomaRAG frontend (Next.js).
...
// BackendSpec configures the BomaRAG backend (FastAPI).
type BackendSpec struct {
	...
	// EncryptionKeySecret references the Secret key for BOMARAG_ENCRYPTION_KEY.
	// If omitted, the operator auto-generates a stable key in the .env file.
...
type LangflowSpec struct {
	...
	// files under flows/ in ABISHAIMWANJA/bomarag at that ref are fetched into
	...
	// PVCReclaimPolicy determines what happens to the Langflow PVC when the BomaRAG CR is deleted.
...
// BomaRAGSpec defines the desired state of an BomaRAG instance.
type BomaRAGSpec struct {
	// InstanceName is a short unique identifier embedded in all resource names, enabling
	// multiple BomaRAG CRs to coexist in the same namespace (e.g. "prod", "staging").
	...
	// static names (bomarag-fe, bomarag-be, bomarag-lf) for backwards compatibility.
	...
	// TargetNamespace is the namespace where all BomaRAG resources are created.
	...
	// Frontend configures the BomaRAG Next.js frontend.
	// +kubebuilder:validation:Required
	Frontend FrontendSpec `json:"frontend"`

	// Backend configures the BomaRAG FastAPI backend.
	// +kubebuilder:validation:Required
	...
	// When enabled, deploys docling-serve and docling-worker alongside BomaRAG.
	// +optional
...
// BomaRAGStatus defines the observed state of an BomaRAG instance.
type BomaRAGStatus struct {
...
// +kubebuilder:resource:shortName=br,scope=Namespaced
...
// BomaRAG is the Schema for the bomarags API.
type BomaRAG struct {
	metav1.TypeMeta   `json:",inline"`
	metav1.ObjectMeta `json:"metadata,omitempty"`

	Spec   BomaRAGSpec   `json:"spec,omitempty"`
	Status BomaRAGStatus `json:"status,omitempty"`
}

// +kubebuilder:object:root=true

// BomaRAGList contains a list of BomaRAG.
type BomaRAGList struct {
	metav1.TypeMeta `json:",inline"`
	metav1.ListMeta `json:"metadata,omitempty"`
	Items           []BomaRAG `json:"items"`
}
