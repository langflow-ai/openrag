...
`source_url`. Sources that already came from the ingestion folder are restored
...
original. Authenticated multipart uploads remain available with or without a
`source_url`.
