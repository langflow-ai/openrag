...
This page provides troubleshooting advice for issues you might encounter when using BomaRAG or contributing to BomaRAG.

## BomaRAG installation fails with unable to get local issuer certificate

If you are installing BomaRAG on macOS, and the installation fails with `unable to get local issuer certificate`, run the following command, and then retry the installation:
...
When you start BomaRAG, `No container runtime found` and `Docker/Podman not ready` errors indicate that BomaRAG cannot find or start a running Docker or Podman machine to run the BomaRAG containers.

Before starting BomaRAG, do the following:
...
3. Unless you are running BomaRAG with a Linux-based VM, such as a Linux-based [WSL](/install-windows) image, start the container VM.
...
Before pulling a missing image, BomaRAG verifies that the image exists in its registry.
If the check fails, BomaRAG stops rather than attempting the pull, and reports an error in the following form:
...
|---|---|---|
| `Image 'IMAGE' not found in registry` | If the registry is reachable and your credentials are valid, then the registry has no image at the given repository path and tag. | Verify that `IMAGE_ORG` matches the organization in your registry, and that the tag from `BOMARAG_IMAGE_TAG` or `BOMARAG_VERSION` exists. This commonly occurs when neither tag variable is set, because BomaRAG then requests the `latest` tag, which many private registries don't populate. |
...
This check only covers the BomaRAG-owned images.
...
For instructions on mirroring the images, see [Deploy BomaRAG from a private registry](/docker#private-registry).

## The terminal session doesn't report available upgrades {#no-upgrade-check-with-private-registry}

The terminal session checks for new BomaRAG versions by querying the Docker Hub API.
...
To track new releases, monitor your own registry or the [BomaRAG releases page](https://github.com/ABISHAIMWANJA/bomarag/releases), and then [upgrade](/upgrade) by mirroring the new images and updating `BOMARAG_IMAGE_TAG` or `BOMARAG_VERSION`.
...
* Increase your Podman or Docker VM's allocated memory.
   ...
   This example, the following commands stop Podman, increase the machine size to 8 GB of RAM (the minimum recommended RAM for BomaRAG), and then restart the VM:
   ...
   You must also restart your BomaRAG services after increasing the container VM's memory.

* Use a CPU-only deployment to reduce memory usage.

   For terminal-managed deployments, you must [start the terminal session with `--tui`](/tui#access-the-tui), such as `uv run bomarag --tui`. Then, click **Status** in the [TUI](/tui), and then click **Switch to CPU mode**.
...
If you need to change the password, you must [reset the BomaRAG services](/manage-services).

## BomaRAG fails to start from the terminal with operation not supported

This error occurs when starting BomaRAG with the terminal's **Start services** option while in the [WSL (Windows Subsystem for Linux)](https://learn.microsoft.com/en-us/windows/wsl/install).

The error occurs because BomaRAG is running within a WSL environment, so `webbrowser.open()` can't launch a browser automatically.

To access the BomaRAG application, open a web browser, and then enter `http://localhost:3000` in the address bar.
...
## BomaRAG reports Docling Serve errors but docling-serve is running

Make sure your `docling-serve` version is compatible with your BomaRAG version.
BomaRAG version 0.4.1 requires `docling-serve` version 1.5.0.

If you started a `docling-serve` container before starting or installing BomaRAG, and the `docling-serve` version is incompatible with your BomaRAG version, you need to remove or restart the incompatible container.
There are several ways to do this:

* Remove the `docling-serve` container that is running the incompatible version, and then [reinstall BomaRAG](/reinstall) with `uv` or `uvx` to automatically start a compatible `docling-serve` container.

* After starting BomaRAG, restart the `docling-serve` container with the required version:
...
By default, BomaRAG requires the following ports to be available on the host machine:

* `3000`: BomaRAG frontend
* `7860`: Langflow service
* `8000`: BomaRAG backend
* `9200`: OpenSearch service
* `5601`: OpenSearch dashboards
* `5001`: Docling service

If the default ports for the BomaRAG frontend (`3000`) or Langflow (`7860`) are already in use on your host machine, you can set the `FRONTEND_PORT` or `LANGFLOW_PORT` [environment variables](/reference/configuration) to map these services to different host ports.
If you set `LANGFLOW_PORT`, you must also set `LANGFLOW_PUBLIC_URL` to use the new port.
To apply the port mapping, [restart the containers](/manage-services#stop-and-start-containers) after editing your BomaRAG `.env` file.

## Upgrade fails due to Langflow container already exists {#langflow-container-already-exists-during-upgrade}

If you encounter a `langflow container already exists` error when upgrading BomaRAG, this typically means you upgraded BomaRAG with `uv`, but you didn't remove or upgrade containers from a previous installation.
...
3. If reinstalling the Langflow container doesn't resolve the issue, then you must [reset all containers](/manage-services) or [reinstall BomaRAG](/reinstall).

4. Retry the [upgrade](/upgrade).

   If no updates are available after reinstalling BomaRAG, then you reinstalled at the latest version, and your deployment is up to date.

## Model availability and performance issues {#model-availability-and-performance-issues}

The following issues relate to the models you can use with BomaRAG and potential performance issues, such as malformed responses and excessive hallucinations.

See also [Chat issues](#chat-issues).

### Language model isn't listed in BomaRAG settings or application onboarding

If your language model isn't listed in the BomaRAG settings or application onboarding, then the model likely doesn't support tool calling, which is required for BomaRAG.
...
If you experience timeouts during ingestion, edit the following environment variables in your [BomaRAG `.env` file](/reference/configuration):
...
This is likely due to a stale `uv` cache when you [install BomaRAG with `uvx`](/install-uvx).

When you invoke BomaRAG with `uvx bomarag`, `uvx` creates a cached, ephemeral environment that doesn't modify your project.
...
1. If an active BomaRAG terminal is running, select the **Exit** option to end the session.
...
2. Clear the `uv` cache:
   ...
   Or clear only the BomaRAG cache:

   ```bash
   uv cache clean bomarag
   ```

3. Invoke BomaRAG to restart the terminal session and regenerate the cache:

   ```bash
   uvx bomarag
   ```

4. Select the **Start services** option to restart the BomaRAG containers.

5. Select the **Open BomaRAG in browser** option, and then retry document ingestion.

In contrast to `uvx`, if you install BomaRAG with `uv`, dependencies are synced directly from your `pyproject.toml` file.
This should automatically install `easyocr` because `easyocr` is included as a dependency in BomaRAG's `pyproject.toml`.
...
If the BomaRAG **Chat** doesn't seem to use your documents correctly, [browse your knowledge base](/knowledge-browse) to confirm that the documents are uploaded in full, and the chunks are correct.
...
The following issues can occur when using the [BomaRAG **Chat**](/chat) feature.
...
Click <Icon name="Library" aria-hidden="true"/> **Knowledge** to confirm that the documents are present in the BomaRAG OpenSearch knowledge base, and then click each document to see how the document was chunked.
...
First, verify that the container VM and the BomaRAG services are running and healthy.

Second, make sure there are no issues with the flow configuration.
If you edited the **BomaRAG OpenSearch Agent** flow, use the [**Restore flow** option](/agents#revert-a-built-in-flow-to-its-original-configuration) to revert the flow to its original configuration.
...
If your model is compatible with BomaRAG, and it isn't exhibiting any obvious errors, then you can use RAG best practices to refine the response quality, such as [knowledge filters](/knowledge-filters) and prompt engineering.

## Cannot detect GPU with Fedora or local Ollama

If your machine has GPU support, but BomaRAG is having problems detecting or using the GPU, you might see errors like the following:
...
12. In a terminal, run BomaRAG, and then start the BomaRAG services with GPU mode enabled.
Or, for self-managed deployments, deploy BomaRAG with the `docker-compose-gpu.yml` file.