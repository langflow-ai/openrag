...
If ingestion fails, click on the **Status** to view the logged error.
...
The <Icon name="Bell" aria-hidden="true"/> **Tasks** icon displays a notification indicator when there are active tasks running.
To open the Tasks menu, click the <Icon name="Bell" aria-hidden="true"/> **Tasks**.

**Active Tasks** displays tasks that are currently processing.
A pending task is queued and waiting to start, a running task is actively processing files, and a processing task is performing ingestion operations. Each active task displays the task ID, start time and duration, and progress showing the number of files processed out of the total files. 
