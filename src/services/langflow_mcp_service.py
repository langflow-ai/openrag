...
REQUIRED_MCP_HEADERS: Dict[str, str] = {
    "X-Langflow-Global-Var-OPENRAG_INGEST_URL": "OPENRAG_INGEST_URL",
    "X-Langflow-Global-Var-OPENRAG_INGEST_TOKEN": "OPENRAG_INGEST_TOKEN",
    "X-Langflow-Global-Var-OPENRAG_INGEST_RUN_ID": "OPENRAG_INGEST_RUN_ID",
    "X-Langflow-Global-Var-OPENRAG_INGEST_BATCH_SIZE": "OPENRAG_INGEST_BATCH_SIZE",
    "X-Langflow-Global-Var-SELECTED_EMBEDDING_MODEL": "SELECTED_EMBEDDING_MODEL",
    "X-Langflow-Global-Var-SELECTED_EMBEDDING_PROVIDER": "SELECTED_EMBEDDING_MODEL_PROVIDER",
    "X-Langflow-Global-Var-OPENSEARCH_URL": "OPENSEARCH_URL",
    "X-Langflow-Global-Var-OPENRAG_LLM_BASE_URL": "OPENRAG_LLM_BASE_URL",
    "X-Langflow-Global-Var-OPENRAG_LLM_TOKEN": "OPENRAG_LLM_TOKEN",
}


class MCPServerURLUpdateError(Exception):
...
class LangflowMCPService:
    ...
    async def patch_mcp_server_url(self, server_name: str) -> str:
        ...
        Also ensures required global variable headers are configured for lf-starter_project.
        ...
        try:
            ...
            if server_name == "lf-starter_project":
                for k, v in REQUIRED_MCP_HEADERS.items():
                    if headers.get(k) != v:
                        headers[k] = v
            ...
            if self._is_convertible_to_streamable_http(current):
                ...
                url, stdio_headers = self._parse_stdio_args(args)
                if stdio_headers:
                    for k, v in stdio_headers.items():
                        if headers.get(k) != v:
                            headers[k] = v
                            headers_updated = True
                if url:
                ...
                url_changed = (patched_url != url)
                if not url_changed and not headers_updated:
                    logger.debug(
                        "MCP server configuration unchanged, skipping patch",
                        server_name=server_name,
                    )
                    return "skipped"
                payload = {"url": patched_url, "headers": headers}
                ...
                for i, arg in enumerate(patched_args):
                    if isinstance(arg, str) and (
                        ...
                        if new_url != arg:
                            ...
                            url_patched = True
                if not url_patched and not headers_updated:
                    logger.debug(
                        "No URL or headers to patch in stdio args, skipping",
                        server_name=server_name,
                    )
                    return "skipped"
                command = current.get("command")
                payload = {"command": command, "args": patched_args, "headers": headers}
                mode = "stdio"
            ...
            if response.status_code in (200, 201):
                logger.info(
                    "Patched MCP server configuration",
                    server_name=server_name,
                    mode=mode,
                )
                return "patched"
                ...
                logger.warning(
                    "Failed to patch MCP server configuration",
                    server_name=server_name,
                    mode=mode,
            ...
            logger.error(
                "Exception while patching MCP server configuration",
                server_name=server_name,
                error=str(e),
            )
            raise

    async def update_all_mcp_server_urls(self, strict: bool = False) -> Dict[str, Any]:
        """Fetch all MCP servers and update their URLs (replacing localhost with LANGFLOW_URL).

        Also converts eligible stdio servers to streamable HTTP mode and ensures required headers.
        ...
        if strict and failed_count > 0:
            raise MCPServerURLUpdateError(
                f"Failed to update MCP server URLs for: {failed_servers}",
                summary={**summary, "failed_servers": failed_servers},
            )
