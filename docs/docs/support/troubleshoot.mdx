...
If Docling ingestion fails with an OCR-related error and mentions `easyocr` is missing, install `easyocr` in your OpenRAG environment.

For project-managed installs, ensure `easyocr` is included in your `pyproject.toml`.
1. From the `pyproject.toml` directory, run:
    ```bash
    uv add easyocr
    ```
2. Start OpenRAG:
...
If you're running OpenRAG with `uvx openrag`, `uvx` creates a cached, ephemeral environment and does not modify your project.
