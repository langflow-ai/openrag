...
| `INSTANA_SECRETS` | `contains-ignore-case:key,pass,secret` | Query-string parameters that the tracer replaces with `<redacted>` before a span is sent, as `<matcher>:<name>[,<name>]` where the matcher is one of `equals`, `equals-ignore-case`, `contains`, `contains-ignore-case`, or `regex`. The default covers credential-shaped parameter names but not free-text ones such as the `q` of `GET /v2/files/search`. See [What gets exported](/reference/observability#what-gets-exported). |
