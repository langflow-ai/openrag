---
title: Deploy BomaRAG with self-managed services
slug: /docker
---
...
To manage your own BomaRAG services, deploy BomaRAG with Docker or Podman.

Use this installation method if you don't want to [use an BomaRAG terminal session](/tui), or you need to run BomaRAG in an environment where terminal interaction is unavailable.
...
1. Clone the BomaRAG repository:
...
4. Create a `.env` file at the root of the cloned repository.
   ...
   You can create an empty file or copy the repository's [`.env.example`](https://github.com/ABISHAIMWANJA/bomarag/blob/main/.env.example) file.
   The example file contains some of the [BomaRAG environment variables](/reference/configuration) to get you started with configuring your deployment.

   ```bash
   cp .env.example .env
   ```

5. Edit the `.env` file to configure your deployment using [BomaRAG environment variables](/reference/configuration).
The BomaRAG Docker Compose files pull values from your `.env` file to configure the BomaRAG containers.
...
6. To enable [OAuth mode or cloud storage connectors](/knowledge-connectors), do the following:
   ...
   3. For each connector, you must register the BomaRAG redirect URIs in your OAuth apps:
...
By default, BomaRAG pulls its container images from Docker Hub.
If your environment can't reach Docker Hub, or your organization requires images to be served from an approved registry, mirror the BomaRAG images to your own registry and then point your deployment at it.
...
1. Mirror the following images to your registry.
   ...
   The BomaRAG containers use five images. Replace `VERSION` with the BomaRAG version you intend to deploy, such as `0.6.4`:

   | Source image | Mirrored path must be |
   |---|---|
   | `docker.io/bomalogic/bomarag-backend:VERSION` | `REGISTRY/ORG/bomarag-backend:TAG` |
   | `docker.io/bomalogic/bomarag-frontend:VERSION` | `REGISTRY/ORG/bomarag-frontend:TAG` |
   | `docker.io/bomalogic/bomarag-langflow:VERSION` | `REGISTRY/ORG/bomarag-langflow:TAG` |
   | `docker.io/bomalogic/bomarag-opensearch:VERSION` | `REGISTRY/ORG/bomarag-opensearch:TAG` |
   ...
   :::tip
   The BomaRAG images are multi-architecture manifests.
...
2. If your registry requires authentication, sign in on the host that runs the containers:
   ...
   BomaRAG checks that each image exists in the registry before it pulls, and that check uses your local credentials.
...
3. In your `.env` file, set the [container image and registry variables](/reference/configuration#container-image-and-registry-settings) for your registry:
   ...
   Set `BOMARAG_IMAGE_TAG` if your mirrored images are tagged differently from the BomaRAG release versions.
   It takes precedence over `BOMARAG_VERSION`.
   If neither variable is set, BomaRAG pulls the `latest` tag, which many private registries don't populate.
...
1. To use the default Docling Serve implementation, start `docling serve` on port 5001 on the host machine using the included script:
   ...
   Port 5001 is required to deploy BomaRAG successfully; don't use a different port.
...
3. Deploy the BomaRAG containers locally using the appropriate Docker Compose configuration for your environment:
...
4. Wait for the BomaRAG containers to start, and then confirm that all containers are running:
   ...
   The BomaRAG Docker Compose files deploy the following containers:

   | Container Name | Default address | Purpose |
   |---|---|---|
   | BomaRAG Backend | http://localhost:8000 | FastAPI server and core functionality. |
   | BomaRAG Frontend | http://localhost:3000 | React web interface for user interaction. |
   ...
   When the containers are running, you can access your BomaRAG services at their addresses.

5. Access the BomaRAG frontend at `http://localhost:3000`, and then continue with the [application onboarding process](#application-onboarding).
