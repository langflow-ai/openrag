...
BomaRAG's knowledge filters help you organize and manage your [knowledge base](/knowledge) by creating pre-defined views of your documents.

Each knowledge filter captures a specific subset of documents based on given a search query and filters.

Knowledge filters can be used with different BomaRAG functionality.
...
When you install BomaRAG, it automatically creates an **BomaRAG docs** filter that includes BomaRAG's default documents.
These documents provide information about BomaRAG itself and help you learn how to use BomaRAG.

When you use the BomaRAG <Icon name="MessageSquare" aria-hidden="true"/> **Chat**, [apply the **BomaRAG docs** filter](#apply-a-filter) if you want to ask questions about BomaRAG's features and functionality.
This limits the agent's context to the default BomaRAG documentation rather than all documents in your knowledge base.

After uploading your own documents, it is recommended that you create your own filters to organize your documents effectively and separate them from the default BomaRAG documents.
...
5. Customize the filter settings.
   ...
   * **Data Sources**: Select specific files and folders to include in the filter.
      ...
      For example, if you upload a folder, you could create a filter that only includes the documents from that folder.
   ...
   * **Connectors**: Filter by [upload source](/ingestion), such as the local file system or a cloud storage connector.
...
In the BomaRAG <Icon name="MessageSquare" aria-hidden="true"/> **Chat**, click <Icon name="Funnel" aria-hidden="true"/> **Filter**, and then select the filter to apply.
Chat filters apply to one chat session only.

You can also use filters when [browsing your knowledge base](/knowledge-browse).
