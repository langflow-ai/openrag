...
## Conversation retention settings {#conversation-retention-settings}

| Variable | Default | Description |
|----------|---------|-------------|
| `OPENRAG_CONVERSATION_TTL_DAYS` | `90` | Number of inactive days to retain conversation metadata before nightly hard deletion. Set to `0` to disable pruning deployment-wide. Administrators can also disable pruning from **Settings > Conversations**. Pruning follows `OPENRAG_STORAGE_MODE`: database rows are removed in `db` mode, JSON entries in `files` mode, and both in `hybrid` mode. |
