...
dump_logs() {
  ...
  "${compose_cmd[@]}" logs langflow 2>&1 | tail -500 || echo "${red}Could not get Langflow logs${nc}"
  echo ""
  echo "${yellow}=== Backend logs (last 500 lines) ===${nc}"
  "${compose_cmd[@]}" logs openrag-backend 2>&1 | tail -500 || echo "${red}Could not get backend logs${nc}"
  echo ""
  echo "${yellow}=== Frontend logs (last 300 lines) ===${nc}"
  "${compose_cmd[@]}" logs openrag-frontend 2>&1 | tail -300 || echo "${red}Could not get frontend logs${nc}"
  echo ""
  echo "${yellow}=== OpenSearch logs (last 300 lines) ===${nc}"
  "${compose_cmd[@]}" logs opensearch 2>&1 | tail -300 || echo "${red}Could not get OpenSearch logs${nc}"
  echo ""
}
