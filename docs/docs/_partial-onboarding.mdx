...
You can use different providers for your language model and embedding model. For example, you can use Anthropic for your language model and OpenAI for embeddings.
:::
...
Choose one LLM provider and complete only those steps:
    ...
    :::info
    Anthropic does not provide embedding models. Select Anthropic for your language model, and then select a different provider for embeddings.
    :::
    ...
    No embedding models are available from Anthropic. Select an embedding provider in the next onboarding step.
    3. To load 2 sample PDFs, enable **Sample dataset**.
    This is recommended, but not required.
    4. Click **Complete**.
    5. Select a provider for embeddings, and choose an embedding model.
