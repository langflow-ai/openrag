#!/usr/bin/env python3
"""Insert BomaRAG custom components into flows/component_index.json."""
...
COMPONENTS_DIR = ROOT / "custom_components" / "bomarag"
...
OPENSEARCH_EXT_ID = f"ext:bomarag:{OPENSEARCH_CLASS}@extra"
...
def llm_component() -> dict:
    ...
    return _component(
        ...
        display_name="BomaRAG LLM",
        description=(
            "Chat model via BomaRAG's OpenAI-compatible /v1 proxy. "
            "Base URL and hop token come from global variables at runtime."
        ),
        module="custom_components.BomaRAG.openai_compatible_llm.OpenAICompatibleLLMComponent",
        ...
        template={
            "api_base": _str_input(
                ...
                "Bound to BOMARAG_LLM_BASE_URL at runtime. Same URL as embeddings.",
                value="BOMARAG_LLM_BASE_URL",
                load_from_db=True,
            ),
            "api_key": _secret_input(
                "api_key",
                "BomaRAG LLM Token",
                "Hop token from BOMARAG_LLM_TOKEN. BomaRAG injects this per Langflow run.",
                value="BOMARAG_LLM_TOKEN",
            ),
            "input_value": {
...
def opensearch_component() -> dict:
    ...
    field_order, outputs, and BomaRAG ingest bindings in sync with the flow.
    Icon stays OpenSearch (not the BomaRAG bundle icon used by the proxy nodes).
    """
...
def embedding_component() -> dict:
    ...
    return _component(
        ...
        module="custom_components.BomaRAG.openai_compatible_embedding.OpenAICompatibleEmbeddingComponent",
        ...
        template={
            ...
            "api_key": _secret_input(
                "api_key",
                "BomaRAG LLM Token",
                "Hop token from BOMARAG_LLM_TOKEN. Same token as the chat component.",
                value="BOMARAG_LLM_TOKEN",
            ),
...
def _compute_sha256(index: dict) -> str:
    ...
    back to a dynamic scan that does not know about the BomaRAG bundle.
    """
...
def main() -> None:
    ...
    for name, bundle in index["entries"]:
        if name in {BUNDLE_NAME, "bomarag"}:
    ...
    entries = [entry for entry in index["entries"] if entry[0] not in {"bomarag", BUNDLE_NAME}]
    entries.append([BUNDLE_NAME, bomarag])
