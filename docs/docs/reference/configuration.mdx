...
| Variable | Default | Description |
|----------|---------|-------------|
| `OPENRAG_ENCRYPTION_KEY` | Automatically generated | Required. 32-byte Base64 AES-256-GCM master key locking all database index payloads. Automatically generated by the TUI/CLI Wizard on initial startup. Cannot be lost or rotated without risking data-loss unless migrated manually. |
| `IBM_AUTH_ENABLED` | `false` | Enabling this feature bypasses the local `OPENRAG_ENCRYPTION_KEY` logic to securely query the IBM Secrets Manager API for your decryption keys natively instead. Requires specific IBM credentials. |
