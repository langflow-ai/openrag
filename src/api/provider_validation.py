...
async def validate_provider_setup(
    ...
    Args:
        provider: Provider name ('openai', 'watsonx', 'ollama', 'anthropic', 'azure_ai_foundry')
        ...
        api_version: API version (optional for azure_ai_foundry).
...
def is_azure_ai_foundry_openai_v1_endpoint(endpoint: str) -> bool:
    """True when `endpoint` targets Foundry's OpenAI-compatible `/openai/v1` route.

    That form takes no dated api-version (see `_build_azure_ai_foundry_url`).
    Shared with `langflow_sync.py` so the AZURE_AI_FOUNDRY_API_VERSION global
    variable it syncs to Langflow matches this same behavior, instead of
    always sending a dated value that this endpoint form rejects.
    """
    if not endpoint:
        return False
