#!/usr/bin/env python3
"""Insert OpenRAG into Langflow's SIDEBAR_BUNDLES (source TS or built JS).
...
index. Register both ``OpenRAG`` and ``openrag`` so either folder casing works.
...
def _has_bundle_name(text: str, name: str) -> bool:
    return (
        re.search(
            rf"\{{(?=[^{{}}]*{_field('display_name', 'OpenRAG')})"
            rf"(?=[^{{}}]*{_field('name', name)})"
            rf"[^{{}}]*\}}",
            text,
        )
...
def _object_for(name: str, *, quoted_keys: bool, compact: bool) -> str:
    if quoted_keys:
        return f'{{"display_name":"OpenRAG","name":"{name}","icon":"OpenRAG"}}'
    if compact:
        return f'{{display_name:"OpenRAG",name:"{name}",icon:"OpenRAG"}}'
    return f'{{ display_name: "OpenRAG", name: "{name}", icon: "OpenRAG" }}'
...
def patch_paths(roots: list[Path]) -> int:
    ...
    if not found_anchor:
        ...
        raise SystemExit(
            f"Could not find a SIDEBAR_BUNDLES anchor to insert OpenRAG. Looked in: {searched}"
        )
    return patched
