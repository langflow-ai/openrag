...
The agent can also use documents that you provide during the chat session.
To limit the knowledge available to the agent, use [filters](/knowledge-filters).

The agent can call specialized tools to extend its capabilities.
...
If you [inspect the flow in Langflow](/agents#inspect-and-modify-flows), you'll see that it is comprised of components that work together to ingest chat messages, retrieve relevant information from your knowledge base, and then generate responses.
...
One or more specialized tools can be attached to the **Tools** port to extend the agent's capabilities. In this case, there are two tools: **Calculator** and **OpenSearch**.
...
* **Calculator** component: Connected to the **Agent** component's **Tools** port, this component lets the agent evaluate arithmetic and quantitative requests with a tool call.
