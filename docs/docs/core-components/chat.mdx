...
After you [upload documents to your knowledge base](/ingestion), you can use the BomaRAG <Icon name="MessageSquare" aria-hidden="true"/> **Chat** feature to interact with your knowledge through natural language queries.

The BomaRAG <Icon name="MessageSquare" aria-hidden="true"/> **Chat** uses an LLM-powered agent to understand your queries, retrieve relevant information from your knowledge base, and generate context-aware responses.
...
To add or change the available tools, you must edit the [**BomaRAG OpenSearch Agent** flow](#flow).

:::tip
Try chatting, uploading documents, and modifying chat settings in the [quickstart](/quickstart).
:::

## BomaRAG OpenSearch Agent flow {#flow}

When you use the BomaRAG <Icon name="MessageSquare" aria-hidden="true"/> **Chat**, the **BomaRAG OpenSearch Agent** flow runs in the background to retrieve relevant information from your knowledge base and generate a response.
...
![BomaRAG Open Search Agent Flow](/img/opensearch-agent-flow.png)
...
When you use the BomaRAG <Icon name="MessageSquare" aria-hidden="true"/> **Chat**, your chat messages are passed to the **Chat Input** component, which then sends them to the **Agent** component for processing.
...
* [**MCP Tools** component](https://docs.langflow.org/mcp-client): Connected to the **Agent** component's **Tools** port, this component can be used to [access any MCP server](https://docs.langflow.org/mcp-server) and the MCP tools provided by that server. In this case, your BomaRAG Langflow instance's [**Starter Project**](https://docs.langflow.org/concepts-flows#projects) is the MCP server, and the [**OpenSearch URL Ingestion** flow](/ingestion#url-flow) is the MCP tool.
This flow fetches content from URLs, and then stores the content in your BomaRAG OpenSearch knowledge base. By serving this flow as an MCP tool, the agent can selectively call this tool if a URL is detected in the chat input.

* [**OpenSearch** component](https://docs.langflow.org/bundles-elastic#opensearch): Connected to the **Agent** component's **Tools** port, this component lets the agent search your [BomaRAG OpenSearch knowledge base](/knowledge). The agent might not use this database for every request; the agent uses this connection only if it decides that documents in your knowledge base are relevant to your query.
...
* [**Text Input** component](https://docs.langflow.org/text-input-and-output#text-input): Connected to the **OpenSearch** component's **Search Filters** port, this component is populated with a Langflow global variable named `BOMARAG-QUERY-FILTER`. If a global or chat-level [knowledge filter](/knowledge-filters) is set, then the variable contains the filter expression, which limits the documents that the agent can access in the knowledge base.
If no knowledge filter is set, then the `BOMARAG-QUERY-FILTER` variable is empty, and the agent can access all documents in the knowledge base.
...
When you use the BomaRAG <Icon name="MessageSquare" aria-hidden="true"/> **Chat**, the **BomaRAG OpenSearch Nudges** flow runs in the background to pull additional context from your knowledge base and chat history.

Nudges appear as prompts in the chat, and they are based on the contents of your BomaRAG OpenSearch knowledge base.
Click a nudge to accept it and start a chat based on the nudge.

Like BomaRAG's other built-in flows, you can [inspect the flow in Langflow](/agents#inspect-and-modify-flows), and you can customize it if you want to change the nudge behavior.
However, this flow is specifically designed to work with the BomaRAG chat and knowledge base.
...
## Integrate BomaRAG chat into an application

You can integrate BomaRAG flows into your applications using the BomaRAG SDKs (recommended) or the Langflow API.

### Use the BomaRAG SDKs (Recommended)

It is strongly recommended that you use the BomaRAG SDKs to interact with BomaRAG programmatically or integrate BomaRAG into an application.
This is because BomaRAG passes specific configuration settings to the Langflow API when it runs a flow or conducts a chat session.
For more information, see the following:

* [Python SDK README](https://github.com/ABISHAIMWANJA/bomarag/tree/main/sdks/python)
* [TypeScript/JavaScript SDK README](https://github.com/ABISHAIMWANJA/bomarag/tree/main/sdks/typescript)
...
However, these snippets don't include BomaRAG-specific configuration settings, so the behavior won't be identical to using BomaRAG directly.

The following example demonstrates how to generate and use code snippets for the **BomaRAG OpenSearch Agent** flow:

1. Open the **BomaRAG OpenSearch Agent** flow in the Langflow visual editor:

   1. In BomaRAG, click <Icon name="Settings2" aria-hidden="true"/> **Settings**.
   2. Click **Edit in Langflow**, and then click **Proceed** to launch the Langflow visual editor.
   3. If Langflow requests login information, enter the `LANGFLOW_SUPERUSER` and `LANGFLOW_SUPERUSER_PASSWORD` from your [BomaRAG `.env` file](/reference/configuration).

2. Optional: If you don't want to use the Langflow API key that is generated automatically when you install BomaRAG, you can [create a Langflow API key](https://docs.langflow.org/api-keys-and-authentication).
This key is used to authenticate with the Langflow API; it doesn't grant access to your entire BomaRAG deployment.
