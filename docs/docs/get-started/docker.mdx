...
7. Optional: To enable the [Langflow integration with Langfuse](https://docs.langflow.org/integrations-langfuse), set the following variables:
   ...
   * **`LANGFUSE_BASE_URL`**: Required for self-hosted Langfuse deployments. Leave empty for Langfuse Cloud.

   `LANGFUSE_HOST` remains supported as a deprecated fallback for backward compatibility. If both endpoint variables are set, `LANGFUSE_BASE_URL` takes precedence.
