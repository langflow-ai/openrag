...
def test_queue_summary_counts_active_files_and_selected_tasks(task_service):
    selected = UploadTask(
        task_id="selected",
        total_files=2,
        file_tasks={
            "pending": FileTask(file_path="pending", status=TaskStatus.PENDING),
            "running": FileTask(file_path="running", status=TaskStatus.RUNNING),
        },
        status=TaskStatus.RUNNING,
    )
    other = UploadTask(
        ...
        total_files=2,
        file_tasks={
            "pending-a": FileTask(file_path="pending-a", status=TaskStatus.PENDING),
            "pending-b": FileTask(file_path="pending-b", status=TaskStatus.PENDING),
        },
    ...
    completed = UploadTask(
        ...
        total_files=1,
        file_tasks={
            "done": FileTask(file_path="done", status=TaskStatus.COMPLETED),
        },
        status=TaskStatus.COMPLETED,
    )
    task_service.task_store["user1"] = {
    ...
    summary = task_service.get_queue_summary("user1", {"selected", "unknown-local"})

    assert summary == {
        "active_tasks": 2,
        "pending_files": 3,
        "running_files": 1,
        "processing_capacity": task_service._worker_count,
        "selected_submitted_tasks": 2,
