...
/** Whether a knowledge overlay row corresponds to a task file path (or its pending placeholder). */
export function taskFileOverlayMatchesPath(
  file: { task_id: string; source_url: string; filename: string },
  taskId: string,
  filePath: string,
  fileName?: string,
): boolean {
  ...
  if (file.source_url === filePath) {
    return true;
  }
  if (file.source_url === pendingTaskFileSourceUrl(taskId, name)) {
    return true;
  }
  return file.filename === name && isPendingTaskFileSourceUrl(file.source_url);
}
...
export function findTaskFileOverlayIndex(
  ...
  return files.findIndex((f) =>
    taskFileOverlayMatchesPath(f, taskId, filePath, fileName),
  );
}
