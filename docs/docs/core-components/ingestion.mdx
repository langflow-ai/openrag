...
Upload documents to your [OpenRAG OpenSearch knowledge base](/knowledge) to populate your knowledge base with unique content, such as your own company documents or research papers.

OpenRAG can ingest knowledge from direct file uploads and cloud storage connectors.
...
## URL ingestion {#url-flow}

URL ingestion is disabled. OpenRAG does not fetch, crawl, or ingest web URLs from chat.

To add web content to your knowledge base, save or export the content as a supported file type, and then use [Ingest local files and folders](#ingest-local-files-and-folders) or [Ingest files from cloud storage](#oauth-ingestion).
