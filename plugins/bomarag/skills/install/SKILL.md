---
name: bomarag_install
description: Plan and execute a minimal BomaRAG installation with requirement drafting, task tracking, configuration guidance, and local verification
---

When the user asks to install BomaRAG, follow this workflow in order.

## Initial assessment phase
Before starting installation, check for an existing BomaRAG installation:
1. Check if `~/.bomarag/tui/.env` exists
2. If it exists:
   - Read the configuration to understand what's already set up
   - Check if services are currently running with `docker compose ps` for the generated BomaRAG compose file, or by filtering on the compose project/labels instead of `docker ps --filter "name=bomarag"`
...
First, create or update `requirements.md` using the official BomaRAG installation documentation:
- Use `https://docs.bomarag.com/install-options` as the source of truth
...
- It is bundled with BomaRAG and managed by `uvx` or `uv`
- It should be automatically started and stopped via `uvx bomarag`
...
- Do not immediately flag failure if `http://localhost:5001/docs` is not ready right after startup; wait and re-check because `docling-serve` may come up after the main BomaRAG services
...
6. If troubleshooting partially started BomaRAG services, stop all services before attempting to start them again
...
When the installation reaches the point where `uvx bomarag` needs to be run:

### What NOT to do:
- Do NOT attempt to run `uvx bomarag` through the agent's shell tool expecting to interact with it
...
2. Provide the exact command to run: `uvx --python 3.13 bomarag`
...
5. **After confirmation**, verify the installation using non-interactive commands:
   - Check container status: `docker ps --filter "name=bomarag"`
...
- **ALWAYS instruct users to run `uvx bomarag` in a separate terminal outside the agent's shell**
...
- **Never attempt to run `uvx bomarag` interactively through the agent's shell**
...
- If BomaRAG services were restarted during troubleshooting, verify they are all healthy after the clean restart
