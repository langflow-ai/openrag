...
| `WATSONX_ENDPOINT_ONPREM` | Not set | HTTPS base URL of the IBM Cloud Pak for Data or Software Hub cluster. |
| `WATSONX_USERNAME_ONPREM` | Not set | Cluster username used with `WATSONX_API_KEY_ONPREM`. |
| `WATSONX_API_KEY_ONPREM` | Not set | Cluster API key exchanged with the username for CPD authorization. |
| `WATSONX_ZEN_API_KEY_ONPREM` | Not set | Pre-encoded `base64(username:api-key)` alternative to the username and API-key variables. |
| `WATSONX_SPACE_ID_ONPREM` | Not set | Deployment space used for inference. |
| `WATSONX_PROJECT_ID_ONPREM` | Not set | Optional project alternative to a deployment space. Kept separate from the IBM Cloud `WATSONX_PROJECT_ID`. |
| `WATSONX_TLS_VERIFY_ONPREM` | `false` | Provider-scoped TLS policy: `false`, `true`, or a mounted CA bundle path. `SSL_VERIFY` no longer controls this provider; `SSL_CERT_FILE` supplies the process trust store when verification is enabled. |
| `RHOAI_ENDPOINT` | Not set | Chat endpoint for the Red Hat OpenShift AI model provider: the predictor Service of the chat `InferenceService`, ending in `/v1`. |
| `RHOAI_EMBEDDINGS_ENDPOINT` | Not set | Embeddings endpoint for the Red Hat OpenShift AI model provider, a second `InferenceService`. Leave unset only if one endpoint serves both chat and embeddings. |
| `RHOAI_API_KEY` | Not set | Kubernetes bearer token for the Red Hat OpenShift AI model provider, sent to both endpoints. Its subject needs `get` on `inferenceservices` in the model namespace. |
| `RHOAI_TLS_VERIFY` | Not set | TLS trust for the Red Hat OpenShift AI endpoints. Set to a CA bundle path, such as the service-serving CA OpenShift projects into every pod (`/var/run/secrets/kubernetes.io/serviceaccount/service-ca.crt`), or `false` to skip verification for a local `oc port-forward` only. Scoped to this provider, so it does not affect other providers. |
| `RHOAI_EMBEDDING_MAX_CONCURRENCY` | `4` | Maximum number of embedding requests OpenRAG sends to the Red Hat OpenShift AI embeddings endpoint at once; further requests wait inside OpenRAG. Keeps a slow (for example CPU-served) model from being queued past the endpoint proxy's upstream timeout, which otherwise returns `502` when several files are ingested together. Set `0` for no limit, or a higher value for GPU-backed deployments. Also editable in **Settings**. |

Newly indexed chunks keep the model name in `embedding_model` for compatibility and also store
`embedding_provider` plus the unambiguous `embedding_space_id` (`provider:model`). Retrieval queries
each indexed space whose provider is configured. Chunks created before this metadata existed remain
searchable: `text-embedding-3-small` is treated as OpenAI, while other legacy model names require
`OPENRAG_LEGACY_EMBEDDING_PROVIDER_MAP`. If a legacy provider cannot be resolved, OpenRAG skips its
vector query but keeps those documents eligible for keyword search.
...
`rhoai` (Red Hat OpenShift AI) is hidden in every run mode as shipped, because
its endpoints and token are specific to one OpenShift cluster. To offer it,
copy `model_providers.yaml`, set `rhoai` to `true` for your run mode, point
`OPENRAG_MODEL_PROVIDERS_CONFIG` at the copy, and restart the backend. The
`RHOAI_*` variables above seed its credentials on first boot; the `models` and
`embedding_models` rows on that entry are the fallback pickers until the
cluster's own `/v1/models` listing has been read.
...
#### Hide specific models {#exclude-models}

A row can also keep ids out of the pickers. `*` and `?` wildcards work, and
matching is against the id as the picker shows it — `gpt-3.5-turbo`, not
`openai/gpt-3.5-turbo`. The id's last path segment is matched too, so a plain
name also covers Azure's regional listings (`eu/gpt-5.6-luna`,
`us/gpt-5.6-luna`); write `eu/*` to reach one region on its own.

Exclusions are per provider row. Azure OpenAI serves the same models as
OpenAI, so a model you want gone from both needs the pattern on both rows.
...
Exclusion is applied last, so it also overrides a `models` entry on the same
provider, and it covers the live `/models/{provider}` lists as well as the
catalogue — otherwise an id hidden in Settings would reappear in onboarding.

Two other mechanisms already run without configuration, so reach for
`exclude_models` only when neither covers you:

| Situation | Handled by |
|---|---|
| The provider has already retired the model | Dropped from the catalogue automatically |
| The provider has announced a retirement within 90 days | Hidden behind a **Show N retiring** row in the picker |
| This deployment does not want the model offered at all | `exclude_models` |
...
| `DASHBOARDS_REGISTRY` | `docker.io` | Registry host for the OpenSearch Dashboards image only. This lets you point Dashboards at a mirror without redirecting the OpenRAG-owned images. The repository path and tag within the registry are fixed, so the mirrored image must be available at `DASHBOARDS_REGISTRY/opensearchproject/opensearch-dashboards:3.8.0`. |
